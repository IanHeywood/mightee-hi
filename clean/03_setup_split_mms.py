...
inms = sys.argv[1]
mslist = glob.glob(inms+'/SUBMSS/*.ms')

# if 'full' in inms:
#     field = inms.split('full_')[-1].split('_L2')[0]
# elif 'l0' in inms:
#     field = inms.split('l0_')[-1].split('_L2')[0]
# else:
#     field = inms.split('l2_')[-1].split('_L2')[0]
...
for subms in mslist:
    ...
    for chan in chans:
        ...
        opms = opdir+'/'+subms.split('/')[-1].replace('.mms','_'+str(chan0).zfill(4)+'-'+str(chan1).zfill(4)+'.mms')
        slurmfile = split_channels(subms,jobname,chan0,chan1,opms)
        if idx == 0:
